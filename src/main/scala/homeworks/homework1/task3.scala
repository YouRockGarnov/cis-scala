...
sealed trait BinaryTree {
  ...
  def size: Int =
    this match {
      case Branch(value, left, right) => left.size + right.size + 1
      case Leaf => 0
    }
  ...
  def take(n: Int): BinaryTree =
    n match {
      case size if size >= this.size => this
      case size => this match {
        case Branch (value, left, right) =>
          val leftBatch = left.take(size)

          if (leftBatch.size >= size) {
            leftBatch
          } else {
            val rightBatch = right.take(size - leftBatch.size - 1)
            Branch(value, leftBatch, rightBatch)
          }
      }
    }
